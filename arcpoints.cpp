...
ARC arcpoints::getArc(Eigen::Vector3d p1, Eigen::Vector3d p2, Eigen::Vector3d p3, float division){
    ...
    arc.lenghtLineA=(p2-p1).norm();
    arc.lenghtLineB=(p3-p2).norm();
    arc.lenghtLineAB=(p3-p1).norm();
    if(arc.lenghtLineA+arc.lenghtLineAB==arc.lenghtLineAB){
        std::cout<<"lines are in same direction (colineair?), abort"<<std::endl;
        return arc;
    }
    ...
    arc.circumFence=(M_PI*(2*arc.radius));

    //! Arc lenght.
    arc.arcLenght=(arc.arcAngleRad/(2*M_PI))*arc.circumFence;
